...
def generate_answer_with_language_structured(query: str, context: str, language: str, history_messages: list[dict]) -> str:
    ...
    # Compose message list - FIXED: Don't add duplicate system message
    messages: list[dict] = []
    
    # If we have history messages, use them (they already include system context)
    if history_messages:
        messages.extend(history_messages)
    else:
        # Only add system prompt if no history exists
